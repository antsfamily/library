...
[z, numA, numAt, objective, distance1, distance2, criterion, times, mses, ISNR] = ...
         csalsa(y, A, mu1, mu2, sigma,...
         'AT', AT, ...
         'StopCriterion', 2, ...
         'True_x', x, ...
         'ToleranceA', tolA,...
...
mse = norm(x-z,'fro')^2 /(N*N);
ISNR_final = 10*log10( norm(y-x,'fro')^2 / (mse*N*N) );
cpu_time = times(end);
